...
float eval_surface(vec2 xz, float t, vec4 p) {
    return p.x * 0.9 * (sin(p.y * xz.x - 2.0 * t) * cos(p.y * xz.y) +
                        cos(p.y * xz.x) * sin(p.y * xz.y + 2.0 * t));
}
...
void main()
{
    ...
    vec3 col_mid  = vec3(0.85, 0.25, 0.75); // Magenta/Rose
    vec3 col_high = vec3(1.00, 0.75, 0.20);
